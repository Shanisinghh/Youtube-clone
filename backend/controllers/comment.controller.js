...
export async function fetchComment(req, res) {
    try {
        const allComments = await Comment.find().populate("userId", "username avatar email");
        if(!allComments){
            return res.status(400).json({message:"Something went wrong"})
        }
        return res.status(200).json({message:"Fetch all comments ",allComments})
    } catch (error) {
...
export async function createComment(req, res) {
  try {
    const newComment = await (await Comment.create({...req.body, user: req.user}))
    if (!newComment) {
      return res.status(400).json({ message: "Something went wrong" });
    }
    return res
